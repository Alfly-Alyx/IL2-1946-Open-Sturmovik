...
| IL-2 Sturmovik 2001 Retail - remaster 1586x992.png | `il2-2001.tga` | **3** |
| 3.png | `background-3.tga` | 1 |
| IL-2 Sturmovik 2001 - jaquette remaster 1586x992.png | `il2-2001-box.tga` | 1 |

Les noms internes courts servent à éviter les espaces et accents dans les chemins du moteur. Le catalogue conserve les noms source exacts et leurs empreintes. Le champ `weight` décrit la préférence choisie ; la configuration enregistrée (`official=il2-2001`) applique le triple passage.
...
## Mise à jour depuis GitHub v1.15 — 11 septembre 2026

À la demande d’Alexis, la branche isolée `codex/rotation-fonds` a intégré `v1.15` depuis GitHub, commit `b3ed8a505ece716a0c5ec1764f4d37c2bfff2f11`. La fusion `3fe2d3bad` s’est terminée sans conflit. Le code et les ressources de rotation sont inchangés. Le checkout partagé reste sur `v1.15` et n’a pas été modifié.

La copie de test conserve le profil 9, 4.09m mods 6DOF, avec les nouveaux EXE et SFS du manifeste. Les onze autres fichiers que le switcher aurait copiés correspondent déjà aux sources du profil. Les fichiers de configuration, les quatre fonds actifs et les sauvegardes de réversibilité sont conservés à l’octet près ; l’état reste en position 3, dernier fond `il2-2001`. Les 1 116 suppressions antérieures de `PaintSchemes/Cache` sont maintenues. Les suppressions et renommages apportés par GitHub sont conservés.

Le switcher valide le profil 9, le module le reconnaît comme moddé, les empreintes EXE/SFS des neuf profils correspondent au manifeste et les **78 contrôles d’installation, désactivation et retrait passent sous PowerShell 5.1**. Aucun jeu n’a été lancé pour cette mise à jour. Les essais visuels précédents restent des preuves historiques portant sur leurs anciens EXE/SFS. Résultats : [`loading-rotation-v115-update.json`](../manifests/loading-rotation-v115-update.json). La sauvegarde exacte et la mise en réserve Git restent dans la copie isolée ; leur inventaire figure dans ce rapport.
...
## Fond officiel : trois passages par cycle

Après la mise à jour GitHub, Alexis a demandé trois apparitions du fond `il2-2001` au lieu de deux. Le cycle contient désormais six passages, de poids **3:1:1:1**. Les quatre images sont nécessaires pour conserver l’absence de répétition immédiate. Le catalogue, le générateur, le helper et la fenêtre de réglage indiquent cette nouvelle fréquence.

Le poids participe maintenant à la signature de l’état. Au prochain lancement, l’ancien cycle de cinq passages sera reconstruit en conservant le dernier identifiant affiché. Une impossibilité de précharger le matériau laisse l’ancien état intact. Les tests couvrent cette migration depuis chacune des cinq positions anciennes, dans les deux modes, ainsi que le passage au cycle suivant.
