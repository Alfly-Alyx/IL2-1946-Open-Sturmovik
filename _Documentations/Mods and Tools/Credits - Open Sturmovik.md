# Crédits — Open Sturmovik

**par la communauté, pour la communauté**

Open Sturmovik réunit des créations de la communauté IL-2 1946. Merci aux
auteurs et aux équipes des mods dont les travaux contribuent au pack.

Liste en cours pour la v1.15, enrichie par le scan des dossiers du jeu et la comparaison aux sources locales. Les contributions et options ci-dessous sont fournies par le pack ; leur présence ne signifie pas qu’elles sont toutes sélectionnées dans chaque profil.

## Mods et contributions

| Mod | Auteur ou collectif | Contribution au pack |
| --- | --- | --- |
| Améliorations issues du forum All Aircraft Arcade (AAA) | Auteurs et contributeurs du forum AAA | Plusieurs améliorations d’Open Sturmovik proviennent des créations partagées sur ce forum. |
| Advanced Engine Management — AOC 1a | II/JG51-Lutz | Gestion avancée des moteurs. |
| Tiger33 Ultimate Sound Mod V3 | Tiger33 | Sélection de sons et de réglages de démarrage des moteurs. |
| WxTech clouds Jan 2023 | WxTech | Ressources visuelles des nuages. [Publication du mod](https://www.sas1946.com/main/index.php?topic=70195.0). |
| Cockpit CW-21 pour 4.09 | Epervier — conversion pour Rebels 409 ; Team Daidalos — crédits du cockpit | Cockpit du CW-21 adapté à la base 4.09. |
| Magister — modèle extérieur | RAF_Magpie | Élément du modèle extérieur retrouvé à l’identique ; [source et périmètre confirmé](../../docs/SCAN_PACKS_CREDITS_V1.15.md). |
| B-29 Silverplate 1.2, Little Boy et Fat Man | Auteurs et équipe détaillés ci-dessous | B-29 Silverplate, cockpit et ressources des bombes, avec adaptations pour Open Sturmovik. |
| 6DOF Tracker 2.0 | sHr | Déplacement du point de vue dans les profils 6DOF du switcher ; attribution aussi reprise dans les [notes HSFX v4](https://mail.mission4today.com/index.php?file=viewtopic&finish=15&name=ForumsPro&start=0&t=7663). |

## B-29 Silverplate

Crédits du paquet d'origine : **1C/Maddox, O_Magpie, Fireball, SAS~Cirx,
MrJolly, Lt.Wolf, Fat Duck, VC-81_BOLTER, O_Leigh, Max_Thehitman, Ranwers,
Wolfighter et Twister**. [Publication et crédits du mod Silverplate](https://www.sas1946.com/main/index.php?topic=7894.0).

## Cartes et textures

| Carte ou contribution | Auteurs | Notice conservée |
| --- | --- | --- |
| Ukraine, été et hiver | Autopilot | [Notice](<../Maps/AP_Ukraine/Lisez-moi - Ukraine beta ete et hiver.txt>) |
| B29 Alley | delvpier | [Notice](<../Maps/B29_Alley/Lisez-moi - installation.txt>) |
| Bessarabia — repeuplement | Zipzapp ; Fly_zo pour l’installateur historique | [Notice](<../Maps/Bessarabia/Lisez-moi - repeuplement par Zipzapp.txt>) |
| Guadalcanal, septembre 1942 | Marco | [Notice](<../Maps/guadal/Lisez-moi - Guadalcanal septembre 1942 version 1-2.txt>) |
| Channel — petite carte Kt_channel | Kapteeni | [Notice](<../Maps/Kt_channel/Lisez-moi - Channel.txt>) |
| Eastafrica | Kapteeni ; Fly_zo pour le correctif | [Notice](<../Maps/Kt_eastafrica/Lisez-moi - Eastafrica beta 0.91.txt>) |
| Lybia N-E / Tobruk | BADA | [Notice](<../Maps/Lybia_N-E/Lisez-moi - Lybia N-E alpha 1.3.txt>) |
| BP Midway et eau du Pacifique | Boosher — carte ; panzerkeil — objets statiques ; Viking — couleur de l’eau | [Notice](<../Maps/midway/Lisez-moi - eau du Pacifique pour BP Midway.txt>) |
| Darwin Small | Neil Lowe | [Notice](<../Maps/NTL_Darwin_Small/Lisez-moi - Darwin Small 1.1.txt>) |
| Alpen | Zipzapp ; JV69_BADA — aérodromes ; Lowfighter — châteaux suisses | [Notice](<../Maps/zip_Alpen/Lisez-moi - Alpen beta 2.txt>) |
| Alpen — textures fsmd_mount3 et champs Bob | Phasmid — relief ; Rus_Andrey — champs | [Notice](<../Maps/zip_Alpen/Lisez-moi - Alpen beta 2.txt>) |
| Mbug Slovakia winter | may-bug — adaptation hivernale ; Slovakia Team — carte d’origine | [Preuves et version adaptée](../../docs/SCAN_CARTES_CREDITS_V1.15.md) |

Les crédits portent sur les contributions indiquées. Ils ne remplacent pas ceux des cartes d’origine ni ceux des autres éléments utilisés.

## Campagnes fournies

| Campagne | Auteur |
| --- | --- |
| [Baltic Sparrows 1946](<../Campaigns/Baltic Sparrows 1946/Lisez-moi - Baltic Sparrows 1946.txt>) | Eldon45 |
| [Schlageter 43](<../Campaigns/Schlageter 43/Lisez-moi - Schlageter 43.txt>) | mandrill7 |
| [Wunderbomber](<../Campaigns/Wunderbomber/Lisez-moi - Wunderbomber.txt>) | Kernow |
| [Kilroy](<../Campaigns/Kilroy/Lisez-moi - Kilroy - version moddee.txt>) | Bowie |
| [Straight Down](<../Campaigns/Straight Down/Lisez-moi - Straight Down.txt>) | Zeus-cat |
| [Two Little DUCs](<../Campaigns/Two Little DUCs/Lisez-moi - Two Little DUCs.htm>) | Extreme_One |
| [Wings over Waves - Kuro Yuri](<../Campaigns/Wings over Waves/Version - Kuro Yuri IJ1.txt>) | PlusWave Expansions / MDi Design GmbH |
| [Wings over Waves - Koryo](<../Campaigns/Wings over Waves/Version - Koryo IJ2.txt>) | PlusWave Expansions / MDi Design GmbH |
| [Wings over Waves - Archer](<../Campaigns/Wings over Waves/Version - Archer US2.txt>) | PlusWave Expansions / MDi Design GmbH |
| [Wings over Waves - Catball](<../Campaigns/Wings over Waves/Version - Catball US1.txt>) | PlusWave Expansions / MDi Design GmbH |

## Variantes et outils fournis

| Mod ou outil | Auteur | Contribution |
| --- | --- | --- |
| Viseurs Schräge Musik du Bf-110G | magot | Variantes de réticules fournies. |
| Gamma Panel | Tomasz Porosiński | Réglage optionnel du gamma. |
| Bombsight Table 2 | WT_Pedropan ; WT_Pitr | Outil de calcul pour le bombardement. |
| HardBall Aircraft Viewer 4.08 | Matt « HardBall » Henderson | Consultation des caractéristiques des avions. |
| IL2 Sticks | FoolTrottel | Configuration des commandes. |
| IL2 JoyControl | Oleg_BS | Réglage des commandes. |
| Mission Mate 6 | CrazySchmidt ; Barbs | Préparation de missions. |
| Quick Mission Tuner 1946 | DiverseWare | Modification de missions. |
| Properties Editor et Difficulty Editor | MadBran | Outils auxiliaires de Quick Mission Tuner. |
| Shift-Rot | Antonio M. | Outil auxiliaire de Quick Mission Tuner. |

Les versions, les notices et les limites de ces options sont relevées dans [l’inventaire des outils](../../docs/SCAN_OPTIONS_CREDITS_V1.15.md).

## Attributions complémentaires

**BombBayDoors Plus 2.5.3 — Zuti et Fireball.** Attribution retrouvée dans une notice du pack BAT. La version exacte du code présent dans Open Sturmovik reste à préciser ; [la source et les correspondances sont conservées ici](../../docs/SCAN_PACKS_CREDITS_V1.15.md).

## Inventaire en cours

Le [scan des dossiers et des ressources](../../docs/SCAN_MODS_V1.15.md) conserve les noms des 130 modules comparés, leurs correspondances et les fichiers restant à attribuer. Le [relevé des packs locaux examinés](../../docs/SCAN_PACKS_CREDITS_V1.15.md) complète ces recherches. La liste sera enrichie avec les auteurs des autres ressources effectivement intégrées. L'attribution des éléments hérités de RMP3 Atmosphere
et de WindConfig v3 reste notamment à préciser.

Les sources, les limites des intégrations partielles et les points à vérifier
sont consignés dans [l'inventaire détaillé des mods](../../docs/CREDITS_MODS.md).
