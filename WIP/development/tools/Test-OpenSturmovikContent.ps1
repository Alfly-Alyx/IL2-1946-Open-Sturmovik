...
$defaultProjectRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..\..'))
...
$chiefExtensions = Join-Path $developmentRoot 'manifests\registries\chief.mod-extensions.ini'
...
$aocManifestPath = Join-Path $developmentRoot 'manifests\aoc-v1.15.json'
...
$kb29pManifestPath = Join-Path $developmentRoot 'manifests\aircraft\kb29p-qmb-v1.15.json'
...
$cwManifestPath = Join-Path $developmentRoot 'manifests\aircraft\cw21-cockpit-v1.15.json'
...
$aaaRestoreManifestPath = Join-Path $developmentRoot 'manifests\aircraft\aaa-community-cockpits-v1.15.json'
...
try {
    $mdsReport = & python -B (Join-Path $developmentRoot 'tools\Test-NoMds.py') --content-root $root --manifest-root $developmentRoot
...
if (-not $ExcludeNuclear) {
    $nuclearManifestPath = Join-Path $developmentRoot 'manifests\effects\nuclear-blast-v1.15.json'
...
$audioManifestPath = Join-Path $developmentRoot 'manifests\audio\tiger33-startup-sounds.json'
...
$allisonManifestPath = Join-Path $developmentRoot 'manifests\audio\allison-v1.15.json'
...
$effectManifestPath = Join-Path $developmentRoot 'manifests\effects\effect-limit-audit.json'
...
$cloudManifestPath = Join-Path $developmentRoot 'manifests\effects\clouds-4.09m-v1.15.json'
...
$sixDofManifestPath = Join-Path $developmentRoot 'manifests\profiles-6dof-v1.15.json'
...
if ($ReportPath) {
    $absoluteReport = if ([IO.Path]::IsPathRooted($ReportPath)) { $ReportPath } else { Join-Path $developmentRoot $ReportPath }
