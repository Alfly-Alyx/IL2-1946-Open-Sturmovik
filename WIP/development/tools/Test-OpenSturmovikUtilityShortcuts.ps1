...
$manifestPath = Join-Path $developmentRoot 'manifests\utilities-v1.15.json'
$installerPath = Join-Path $developmentRoot 'tools\Install-OpenSturmovikUtilityShortcuts.ps1'
$initializerPath = Join-Path $developmentRoot 'tools\Initialize-OpenSturmovikUtilities.ps1'
$updateFinalizerPath = Join-Path $developmentRoot 'tools\Complete-OpenSturmovikV115Update.ps1'
...
if ($manifest.fullInstaller.script -ne 'installer\Open_Sturmovik_1.15.iss' -or
    [int]$manifest.fullInstaller.shortcutCount -ne 11 -or
    $manifest.fullInstaller.autoLaunchUtilities -ne $false) {
    throw 'Description incoherente de l installateur complet v1.15.'
}
