...
def main():
    ...
    parser.add_argument('--game-root', type=Path, help='Include the test installation common SFS tables and the version SFS stored in each profile directory (read-only).')
    ...
    sfs = load('switcher_sfs', development / 'tools/Analyze-Sfs.py')
    air = load('switcher_air', development / 'tools/Audit-AirIniAircraft.py')
    # Only index the loose directory; never call the dump fallback.
    resolver = air.ClassResolver(repo / 'Files', repo / 'WIP/temporaire/unused-no-dump')
    resolver.index_loose()
    manifest = json.loads((development / 'manifests/switcher-v1.15.json').read_text('utf-8-sig'))
    ...
    for profile in manifest['profiles']:
        ...
        archive_path = repo / profile['filesArchivePath']
        ...
        if args.game_root:
            for item in manifest['versionFiles'][profile['version']]:
                if item['file'].lower().endswith('.sfs'):
                    source = repo / profile['directoryPath'] / item['file']
                    with sfs.SfsArchive(source) as archive:
        ...
        result = {
            'profile': profile['number'], 'version': profile['version'],
            ...
            'scope': 'Files loose classes, profile files.SFS, common SFS and version SFS stored in the profile directory' if args.game_root else 'Files loose classes and this profile files.SFS only; not all engine resource containers',
        }
