...
if ([string]::IsNullOrWhiteSpace($BaselineRoot)) {
    $BaselineRoot = Join-Path $PSScriptRoot '..\..\tests\installations\IL 2 Sturmovik 1946 test'
}
...
foreach ($relative in @(
    ...
    '_Game Switcher\4.09m Mods ON (NO 6DOF)\files.SFS'
)) {
...
$modSfs = Join-Path $resolvedGame '_Game Switcher\4.09m Mods ON (NO 6DOF)\files.SFS'
