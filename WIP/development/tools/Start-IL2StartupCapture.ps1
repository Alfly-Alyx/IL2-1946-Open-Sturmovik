...
if ([string]::IsNullOrWhiteSpace($ReferenceRoot)) {
    $ReferenceRoot = 'D:\Projets\GITHUB\#res\IL2 1946\0 - ORIGINAL GAMES DO NOT MODIFIED\Il-2 Sturmovik 1946 _4.09m'
}
if ([string]::IsNullOrWhiteSpace($ResultsRoot)) {
    $ResultsRoot = Join-Path $PSScriptRoot '..\..\tests\captures\startup'
}
if ([string]::IsNullOrWhiteSpace($ProcmonPath)) {
    $ProcmonPath = Join-Path $PSScriptRoot '..\..\dependances\sdk\test-tools\sysinternals\Procmon64.exe'
}
if ([string]::IsNullOrWhiteSpace($ProcDumpPath)) {
    $ProcDumpPath = Join-Path $PSScriptRoot '..\..\dependances\sdk\test-tools\sysinternals\procdump\procdump.exe'
}
if ([string]::IsNullOrWhiteSpace($FrameCapturePath)) {
    $FrameCapturePath = Join-Path $PSScriptRoot '..\..\dependances\sdk\test-tools\FrameCapture.exe'
}
...
$runtimeFixPlan = Join-Path $repositoryRoot 'WIP\tests\plans\runtimefix-20260906.json'
