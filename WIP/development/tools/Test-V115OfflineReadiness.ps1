...
$contentValidator = Join-Path $developmentRoot 'tools\Test-OpenSturmovikContent.ps1'
...
try {
    $utilityResult = & (Join-Path $developmentRoot 'tools\Test-OpenSturmovikUtilityShortcuts.ps1') -RepositoryRoot $root
    if ($utilityResult.Result -eq 'PASS' -and $utilityResult.ShortcutCount -eq 11 -and -not $utilityResult.DesktopModified) {
        Add-Result 'Raccourcis v1.15' PASS 'Onze raccourcis Bureau valides ; aucun Bureau modifie pendant le controle.'
    }
    else {
        Add-Result 'Raccourcis v1.15' FAIL 'Le controle des onze raccourcis ne retourne pas le resultat attendu.'
    }
}
catch {
...
try {
    $diagnosticResult = & (Join-Path $developmentRoot 'tools\Test-OpenSturmovikDiagnostics.ps1')
...
try {
    $switcherResult = & (Join-Path $developmentRoot 'tools\Test-OpenSturmovikSwitcher.ps1') -RepositoryRoot $root
...
$aocPath = Join-Path $developmentRoot 'manifests\aoc-v1.15.json'
...
$campaignPath = Join-Path $developmentRoot 'docs\CAMPAGNE_FINALE_V1.15.md'
