...
- Jeu de reference, lecture seule : `C:\Users\Alexis\DATA\Projets\GITHUB\IL2-1946-Open-Sturmovik\D:\Projets\GITHUB\#res\IL2 1946\0 - ORIGINAL GAMES DO NOT MODIFIED\Il-2 Sturmovik 1946 _4.09m`.
...
## Affichage et fonds de chargement distribués en 1.15

Le `conf.ini` livré n'impose pas la résolution de la machine de préparation. Le modèle `_Game Switcher/conf.ini` contient un repli 1024 × 768. L'installeur, le raccourci direct du jeu et le raccourci du switcher exécutent `_Game Switcher/Set-OpenSturmovikNativeResolution.ps1`, qui inscrit dans le fichier racine la résolution active de l'écran principal Windows avant le lancement. Le jeu démarre en plein écran avec `FullScreen=1`, `ChangeScreenRes=1` et `SaveAspect=0`. L'implantation et les limites sont consignées dans [l'audit de conf.ini](AUDIT_CONF_INI_HISTORIQUE.md). Le journal du test du 13 septembre confirme `Size: 1920x1080`. Les menus utilisent toutefois une géométrie logique 1024 × 768 au moyen de `x1024(...)` et `y1024(...)`, ce qui explique que leurs polices et commandes paraissent grandes sans réduire la résolution du rendu. Aucun correctif de cette échelle interne n'est inclus dans l'installeur.

Les fonds de chargement du jeu sont choisis dans le moteur Java par les classes libres `Files/B96FAC8E2C4DDBE0` et `Files/F35AD7F42DE76ABC`. Le cycle, les fréquences, l'état persistant, les verrous, le retour au fond historique et la procédure de retrait sont décrits dans [Rotation des fonds de chargement](ROTATION_FONDS_CHARGEMENT.md). Les huit PNG de l'assistant d'installation constituent un système séparé piloté par la progression de copie.
