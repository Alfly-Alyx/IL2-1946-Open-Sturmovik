...
Le profil d'essai `conf.max.ini` a ete retire du pack. Les essais deja realises avec ce profil ont etabli que les extensions NVIDIA (`TexEnvCombine4NV`, `DepthClampNV`, `TextureShaderNV`) devaient rester desactivees comme valeur generique. Le chemin securise accepte par le moteur 4.09m utilisait aussi `HardwareShaders=0`, `Forest=2` et `LandGeom=2`. Le second essai Intel a isole `LandGeom=3` comme seule valeur reecrite : la documentation du patch 4.09m reserve cette portee de 72 km au mode Perfect, alors que `LandGeom=2` conserve la portee normale de 36 km. Le troisieme essai a confirme une empreinte `conf.ini` inchangee.
