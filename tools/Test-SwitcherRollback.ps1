...
$allowedGame = [IO.Path]::GetFullPath((Join-Path $repo 'WIP/test-installations/IL 2 Sturmovik 1946 test'))
...
$report = [ordered]@{
    ...
    expectedExitCode = 6
    actualExitCode = $null
    fifteenActiveFilesRestored = $false
...
try {
    ...
    try {
        $command = 'call "' + $bat + '" --apply 1 standard'
    ...
    $output | Set-Content -LiteralPath (Join-Path $reportRoot 'switcher-output.log') -Encoding UTF8
    ...
    $report.fifteenActiveFilesRestored = $true
