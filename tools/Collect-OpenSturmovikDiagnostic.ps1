...
$startedUtc = [DateTime]::Parse([string]$session.started_utc).ToUniversalTime()
$endedProperty = $session.PSObject.Properties['ended_utc']
$endedUtc = if ($null -ne $endedProperty -and -not [string]::IsNullOrWhiteSpace([string]$endedProperty.Value)) {
    [DateTime]::Parse([string]$endedProperty.Value).ToUniversalTime()
}
